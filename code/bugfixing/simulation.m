...
% civilians created to replace dead gang members
global civiliansCreated
civiliansCreated = 0;
...
for timestep=1:Tstep
    ...
    global calcnoag
    calcnoag = (initnoag-[sum(kills),0]+[0,0,civtogangs',0] + [0, civiliansCreated - sum(civtogangs), 0, 0, 0]);
    if(~isequal(calcnoag, noag'))
        error('summe falsch')
    end
end
%% close video
